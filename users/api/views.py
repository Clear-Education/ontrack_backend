...
def change_password(request):
    ...
    if serializer.is_valid():
        ...
        request.user.auth_token.delete()
        return Response(data={'detail':'Successful change of password, please log back in with your new credentials'}, status=status.HTTP_200_OK)
    else:
...
class UsersViewSet(viewsets.ModelViewSet):
    ...
    @swagger_auto_schema(
        responses={200: serializers.ListUserSerializer(many=True)},
    )
    ...
    def destroy(self, request, pk=None):
        ...
        if not retrieved_user.is_active:
            return Response(data={'detail': 'El usuario ya se encuentra inactivo'}, status=status.HTTP_400_BAD_REQUEST)
        if retrieved_user == request.user:
            ...
            return Response(status=status.HTTP_200_OK)
        if retrieved_user.institucion != request.user.institucion:
            return Response(data={'detail': 'No puede borrar un usuario que no pertenece a su institución'}, status=status.HTTP_400_BAD_REQUEST)
    ...
    def alta(self, request, pk=None):
        ...
        retrieved_user = get_object_or_404(User, pk=pk)
        if retrieved_user.institucion != request.user.institucion:
            return Response(data={'detail': 'No puede modificar a un usuario de otra institución'}, status=status.HTTP_400_BAD_REQUEST)
        if retrieved_user.is_active:
            return Response(data={'detail': 'El usuario ya se encuentra activo'}, status=status.HTTP_400_BAD_REQUEST)
